...
Validates:
    ...
    - check_operating_point: criterion F_hook <= envelope (nominal capacity,
      no design factor); new field names.
...
class TestEnvelopeResultFields:
    ...
    def test_array_lengths_match_n_points(self):
        n = 150
        r = compute_envelope(BSP6_05, n_points=n)
...
class TestPipeCurve:
    ...
    def test_pipe_curve_same_for_all_connections(self):
        ...
        r_btc = compute_envelope(BTC6_30)
        r_bsp = compute_envelope(BSP6_05, n_points=200)
...
class TestEnvelopeIsMin:
    ...
    def test_envelope_equals_min_pointwise(self):
        r = compute_envelope(BTC6_30, **_SJ)
...
class TestBTCWithParams:
    ...
    def test_btc_envelope_below_pipe_at_operating_torque_for_btc605(self):
        ...
        r = compute_envelope(BTC6_05, **_SJ)
...
class TestBTCWithoutParams:
    def test_btc_without_params_ftq_is_none(self):
        r = compute_envelope(BTC6_30)  # no overrides
...
class TestCheckOperatingPoint:
    ...
    def test_utilization_consistent_with_envelope_position(self):
        ...
        pt_over = check_operating_point(BSP6_05, 20_000.0, 700.0)
        assert pt_over.utilization > 1.0
        assert pt_over.hook_load_kips > pt_over.envelope_kips
        assert not pt_over.safe

        pt_under = check_operating_point(BSP6_05, 20_000.0, 300.0)
        assert pt_under.utilization < 1.0
        assert pt_under.hook_load_kips < pt_under.envelope_kips
        assert pt_under.safe
